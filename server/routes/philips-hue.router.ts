...
let philipsHue = PhilipsHueController.getController();
...
philipsHueRouter.put('/lights/:ip/:username/:lightId', (req: Request, res: Response, next: NextFunction) => {
  philipsHue.setLightState(req.params.ip, req.params.username, req.params.lightId, req.body)
    .then(lights => res.json(lights))
    .catch(error => {
      res.statusCode = 400;
      res.json(error);
